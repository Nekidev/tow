...
pub async fn run(args: ClientArgs) -> anyhow::Result<()> {
    ...
    let udp_task = tokio::spawn(run_udp(args.from, request.clone()));
...
async fn run_udp(client_address: SocketAddr, request: Request) -> anyhow::Result<()> {
    ...
    tokio::spawn(monitor_udp_addresses(registry.clone()));
    ...
    loop {
        ...
        if let Some(tx) = registry.get(&address) {
            if tx.datagram_tx.clone().send(datagram.clone()).await.is_err() {
...
async fn monitor_udp_addresses(registry: Registry) -> anyhow::Result<()> {
    loop {
        let table = {
...
async fn handle_udp_connection(
    ...
    let incoming_task = proxy::ws_to_udp(&mut ws_rx, udp.clone(), client_address);
    let outgoing_task = proxy::udp_to_ws(datagram_rx, &mut ws_tx);
